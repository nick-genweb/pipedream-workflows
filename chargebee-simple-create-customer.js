  ...
  async run({ steps, $ }) {
    ...
    try {
      ...
      const response = await axios($, {
        method: "POST",
        url: `https://generation-web.chargebee.com/api/v2/customers`,
        auth: {
      ...
      if (errorCode === "clear_sandbox_data_block_operations") {
        console.log("⏳ Chargebee sandbox is currently being reset. Please wait a few minutes and try again.");
        return {
          status: 503,
          headers: {
            "Content-Type": "application/json"
          ...
          body: {
            success: false,
            error: {
              code: "SANDBOX_RESETTING",
              message: "Chargebee sandbox is currently being reset. Please wait a few minutes and try again.",
      ...
      if (errorCode === "duplicate_entry" || error.response?.status === 422) {
        ...
        return {
          ...
          body: {
            ...
            error: {
              ...
              message: "A customer with this email address already exists in Chargebee",
              details: error.response?.data
            }
          }
        };
